...
Frame Stack Construction: Insert M3 heat-set brass inserts into the upper canopy. Bolt the four 4 mm carbon fiber arms between the lower chassis plate and mid-deck using aluminum standoffs.
Gearbox Integration: Press fit the brass worm gear onto the micro servo output shaft. Align the 30-tooth steel gear inside the gearbox housing, securing it with M2 button head screws.
Suspension Linkage Assembly: Slide the linear coil spring and piston rod into the SLA-printed cylinder. Secure the assembly to the carbon linkage arm using M2 pins.
Wheel & Hub Mounting: Press dual 3 x 6 x 2.5 mm bearings into each wheel rim. Mount the wheel hub assemblies to the articulating arms via aluminum hex adapters.
Cage Mounting & Wiring: Connect the PA12-CF cage segments using the 95A TPU hinges. Mount the full assembly onto the frame using the quick-release stainless steel pin mounts. Route servo wires directly to the auxiliary outputs on your flight controller.
---
💻 Firmware Setup
Flight Controller Setup: Flash Betaflight or ArduPilot target configuration located in `/firmware/betaflight/config.cli`.
Servo Output Allocation: Map Aux Channels 1 & 2 to trigger the dual transformation servos:
PWM 1000us: Flight / Rolling Mode (Fully Enclosed Cage)
PWM 2000us: Crawling Mode (Expanded Cage, Exposed Wheels)
Calibrate Gear Lock: Adjust endpoint values to ensure the worm gear fully locks the sector gear without stalling the micro servos.
---
📜 License
